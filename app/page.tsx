  ...
  return (
    <div className="px-4 md:px-6 py-4 md:py-6 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col space-y-4 md:space-y-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">套利机会</h1>
              ...
              <Table>
                ...
                <TableBody>
                  {opportunities.map((opportunity) => (
                    <TableRow key={`desktop-${opportunity.id}`}>
        ...
        <div className="md:hidden space-y-4">
          {opportunities.map((opportunity) => (
            <Card key={`mobile-${opportunity.id}`} className="overflow-hidden">
              <CardContent className="p-4 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    {(!opportunity.withdrawEnabled || !opportunity.depositEnabled) ? (
                      <Badge variant="destructive" className="flex items-center gap-1">
                        <AlertTriangleIcon className="h-3 w-3" />
                        <span>{getOpportunityStatus(opportunity)}</span>
                      </Badge>
                    ) : (
                      <Badge variant="outline" className="bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20">
                        正常
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="space-y-1">
                    <div className="text-muted-foreground">买入交易所</div>
                    <div>{opportunity.buyExchange}</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-muted-foreground">卖出交易所</div>
                    <div>{opportunity.sellExchange}</div>
                  </div>
                </div>
                
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="space-y-1">
                    <div className="text-muted-foreground">价格差异</div>
                  ...
                  <div className="space-y-1">
                    <div className="text-muted-foreground">链</div>
                    <div>{opportunity.chain}</div>
                  </div>
                </div>
